...
- **Logo:** `empresas.logo_url` existe no schema mas está vazio — não consegui extrair o arquivo de imagem que o usuário colou no chat pra fazer upload. Sem `logo_url`, o cabeçalho cai num fallback de texto (nome da empresa em navy). **Pendência: usuário precisa subir o logo real** (ex: no bucket `documentos` ou qualquer URL pública) e setar `empresas.logo_url` pra aparecer no PDF.
