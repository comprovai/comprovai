...
## Manual de Procedimentos COM-PROC-001 (2026-07-20, item 7) — reconstruído
**Primeira tentativa foi pro lixo:** gerei um PDF (`@react-pdf/renderer`, mesmo estilo da Nota de Débito) baseado só no texto extraído via `pdftotext` do modelo real. O usuário rejeitou — "totalmente fora do padrão" — e mandou o **HTML de verdade** do modelo (`SUP.PROC.01 - Manual de Procedimentos Suprimentos REV2.0 - PADRÃO SGI.html`, mesma pasta do PDF). Lição: extração de texto de PDF perde toda a formatação; pra replicar um padrão visual de verdade, preciso do arquivo fonte (HTML/DOCX), não só do PDF renderizado.

**O que o "padrão SGI" realmente é:** uma página HTML autocontida (não um documento pra imprimir) — cabeçalho com gradiente azul (`#1e3a8a` → `#3b82f6`) e logo da empresa, menu lateral fixo (`position: fixed; left:0`) com sumário em âncoras, cards de seção com sombra, tabelas com header azul, listas com checkmark verde, caixas de destaque (azul) e aviso (âmbar), botão "voltar ao topo" flutuante, rodapé escuro com a linha `Código | Data de Emissão | Versão | Revisão`, e CSS de impressão (`@media print` esconde sidebar/botão) — ou seja, o mesmo HTML serve pra tela E pra virar PDF via print do navegador, não precisa de lib de PDF nenhuma. Esse é o padrão usado em TODOS os manuais internos da Consuldata (vi outros na mesma pasta: FleetCheck, manual do módulo de compras do OMIE), não é específico de Suprimentos — cores/fontes são a identidade do **padrão de documentação interna da empresa**, não da marca do Comprovai.

**Terminologia certa do cabeçalho de controle** (diferente do que inventei na v1): `Tipo` / `Código` / `Data Elaboração` / **`Autor(a)`** / `Versão da Revisão` / **`Revisor(a)`** / `Data Revisão` / **`Aprovação`** / `Data Aprovação` — não é "Elaborado/Verificado/Aprovado Por".

- **`src/app/manual/page.tsx`** — página real do Next, fora de `/app/app/*` de propósito (o template já tem seu próprio header+sidebar+footer; encaixar dentro do `AppLayout` normal do Comprovai geraria sidebar dentro de sidebar, já que a sidebar do template é `position:fixed`). Guard de auth manual (mesmo padrão do `AppLayout`: `createClient()` + `getUser()` + `redirect`), e `middleware.ts` teve o matcher atualizado pra incluir `/manual` (senão a sessão Supabase não renova nessa rota).
- **CSS injetado via `dangerouslySetInnerHTML`, não `<style>{texto}</style>`** — essa segunda forma causa mismatch de hidratação real (SSR escapa aspas do CSS como `&#x27;`, o client não, React descarta o HTML do servidor e re-renderiza tudo do zero). Também: **nunca** coloque `<style>...</style>` (a tag literal) dentro de um comentário JSX `{/* ... */}` — o parser SWC quebra com "Expression expected" mesmo sendo comentário válido em JS puro.
- **Seção "Responsáveis e Aprovadores" é ao vivo**, não estática: busca `usuarios` (nome/role/email/ativo) da empresa direto no Supabase a cada carregamento da página — mais correto que o próprio modelo original da Consuldata, que tem uma tabela de e-mails hardcoded (com typos de domínio no próprio arquivo deles: `consultadata.com.br` vs `consuldata.com.br`) que só fica desatualizada com o tempo.
- **Cadeia de assinatura** (sinalizado no [[TASKS]] pra confirmação): Autor(a) Junior Lopes, Revisor(a) Laryssa Victoria, Aprovação Igor Tenório (Diretor Infraestrutura) — mesma cadeia usada no `SUP-PROC-01` real, não inventei nomes novos.
- **"Busca inteligente no manual"**: mesmo conteúdo (`MANUAL_COMPLETO` de `conteudo-manual.ts`) entra no prompt de `/api/chat` quando o usuário está autenticado — sem página de busca separada.
- **Arquivos apagados** (a tentativa 1, errada): `src/components/documentos/ManualDocument.tsx` e `src/app/api/manual/route.ts`.
- **Armadilha de teste encontrada:** depois de editar o arquivo, a aba do browser que já tinha carregado a página antes acumula um bundle JS antigo em cache — mesmo com o servidor compilando 200 OK de verdade e `tsc`/`eslint` limpos, a aba antiga mostrava erro de hidratação/sintaxe fantasma. Abrir uma aba nova (`tabs_create`) resolveu — os erros eram 100% stale, não reais. Vale lembrar disso antes de gastar tempo debugando um "erro" que só existe numa aba já carregada há muitas edições.
- Logo da Consuldata (mesma URL já usada na Nota de Débito) não carregou no teste desta sessão — rede externa bloqueada neste sandbox, não é bug do código.
